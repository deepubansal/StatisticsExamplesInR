...
qplot(x = Score1, y= Score2, data = rawData, geom = "point", size=2 )
r <- cor(rawData$Score1, rawData$Score2, use = "pairwise.complete.obs", method="pearson")

############## correlation- hpothesis testing##########################
cor.test(rawData$Score1, rawData$Score2)  ########  CI does not contain 0 ########
############## Regression - line of best fit #####################
regModel<-lm(Score1~Score2, rawData)

################ Drawing regression line on plot#############################
qplot(x = Score1, y= Score2, data = rawData, geom = "point" ) + geom_abline(reg=regModel, lwd=2)
